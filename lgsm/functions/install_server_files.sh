...
fn_install_server_files_steamcmd(){
	...
	while [ "${counter}" == "0" ]||[ "${exitcode}" != "0" ]; do
		...
		if [ "${counter}" -le "10" ]; then
			...
			if [ "${counter}" -le "4" ]; then
				if [ "${appid}" == "90" ]; then
					${unbuffer} ${steamcmdcommand} +@sSteamCmdForcePlatformType windows +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_set_config 90 mod "${appidmod}" +app_update "${appid}" -beta "${branch}" +quit
					local exitcode=$?
				else
					${unbuffer} ${steamcmdcommand} +@sSteamCmdForcePlatformType windows +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_update "${appid}" -beta "${branch}" +quit
					local exitcode=$?
				fi
			elif [ "${counter}" -ge "5" ]; then
				if [ "${engine}" == "goldsource" ]; then
					${unbuffer} ${steamcmdcommand} +@sSteamCmdForcePlatformType windows +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_set_config 90 mod "${appidmod}" +app_update "${appid}" -beta "${branch}" validate +quit
					local exitcode=$?
				else
					${unbuffer} ${steamcmdcommand} +@sSteamCmdForcePlatformType windows +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_update "${appid}" -beta "${branch}" validate +quit
					local exitcode=$?
				fi
			fi
	...
	if [ "${engine}" == "goldsource" ]; then
		...
		while [ "${counter}" -le "4" ]; do
			counter=$((counter+1))
			${unbuffer} ${steamcmdcommand} +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_set_config 90 mod "${appidmod}" +app_update "${appid}" "${branch}" validate +@sSteamCmdForcePlatformType windows +quit
			local exitcode=$?
		done
	fi
}
